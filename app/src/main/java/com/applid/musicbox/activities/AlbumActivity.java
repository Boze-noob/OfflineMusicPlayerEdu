...
public class AlbumActivity extends AppCompatActivity implements OnClickListen {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        songalbumAdapter = new SongAlbumAdapter(MainActivity.al.get(position),this);
        System.out.println(MainActivity.al.get(position).get(0).getArtist());
        textView1.setEllipsize(TextUtils.TruncateAt.MARQUEE);
        textView1.setText(MainActivity.al.get(position).get(0).getArtist());
        String size=(MainActivity.al.get(position).size())+"";
        textView2.setText(size);

        Glide.with(this)
                .load(ContentUris.withAppendedId(Uri.parse("content://media/external/audio/albumart"), MainActivity.al.get(position).get(0).getAlbumID()).toString())
                .thumbnail(0.2f)
                .centerInside()
